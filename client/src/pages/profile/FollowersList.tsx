...
export default function FollowersList() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get(`/profile/${userId}/followers`).then((res) => setItems(res.data.followers || [])).finally(() => setLoading(false));
  }, [userId]);
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {items.length > 0 ? (
          ...
          <div className="text-center py-14 bg-white border border-dashed border-slate-200 rounded-2xl">
            ...
            <p className="text-slate-500 text-sm font-medium">No followers yet.</p>
          </div>
        )}
      </div>
    </div>

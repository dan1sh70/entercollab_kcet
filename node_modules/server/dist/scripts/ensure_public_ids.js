...
async function addColumnIfMissing(table) {
...
async function backfillPosts() {
    const rows = await prisma.$queryRawUnsafe(`SELECT id, type FROM posts WHERE public_id IS NULL;`);
    for (const r of rows) {
        ...
        for (let i = 0; i < 5; i += 1) {
            try {
                ...
                if (i === 4)
                    throw new Error(`Failed to set publicId for post ${r.id}`);
            }
        }
    }
}
async function backfillResearch() {
    const rows = await prisma.$queryRawUnsafe(`SELECT id FROM research_papers WHERE public_id IS NULL;`);
    for (const r of rows) {
        for (let i = 0; i < 5; i += 1) {
            try {
                ...
                if (i === 4)
                    throw new Error(`Failed to set publicId for researchPaper ${r.id}`);
            }
        }
    }
}
...
//# sourceMappingURL=ensure_public_ids.js.map
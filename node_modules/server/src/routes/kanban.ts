...
router.get('/projects/:projectId/board', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/boards/:boardId/tasks', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { title, column_id } = req.body;
    if (!title || !column_id) { res.status(422).json({ error: 'Title and column_id are required.' }); return; }
...
router.post('/tasks/:taskId/move', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { column_id, order_index } = req.body;
    await prisma.boardTask.update({
      where: { id: parseInt(req.params.taskId as string) },

...
router.patch('/:id', authMiddleware, upload.single('image'), async (req: AuthRequest, res: Response) => {
  try {
    const event = await prisma.event.findUnique({ where: { id: parseInt(req.params.id as string) } });
    if (!event || event.userId !== req.user!.id) { res.status(403).json({ error: 'Unauthorized' }); return; }
    ...
    if (event_date) data.eventDate = new Date(event_date);
    if (max_attendees) data.maxAttendees = parseInt(max_attendees);
...
router.post('/:id/register', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (existing) { res.status(422).json({ error: 'Already registered.' }); return; }
...
router.delete('/:id/unregister', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const eventId = parseInt(req.params.id as string);

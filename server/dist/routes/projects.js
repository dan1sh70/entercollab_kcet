...
import { createNotification, removeProjectLikeNotification, emitProjectLikes, emitProjectComment, emitProjectRefresh, emitChatRoomsRefresh, } from '../services/notificationService.js';
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const where = { type: 'project', status: 'published', visibility: 'public', deletedAt: null };
        if (tag)
            where.tags = { contains: `#${String(tag).replace(/^#/, '')}` };
        if (category)
        ...
        const bookmarkRows = postIds.length > 0
    ...
    try {
        ...
        const project = await prisma.post.create({
            data: {
                ...
                tags: JSON.stringify(tagsArr.map((t) => `#${t.replace(/^#/, '')}`)),
                image: imagePath,
            },
...
router.get('/:id', authMiddleware, async (req, res) => {
    try {
        const project = await prisma.post.findFirst({
            where: { id: parseInt(req.params.id), type: 'project' },
            include: {
        ...
        const isMember = project.userId === req.user.id ||
...
router.patch('/:id', authMiddleware, upload.single('image'), async (req, res) => {
    try {
        const project = await prisma.post.findFirst({ where: { id: parseInt(req.params.id), type: 'project' } });
        ...
        if (tags) {
            ...
            data.tags = JSON.stringify(tagsArr.map((t) => `#${t.replace(/^#/, '')}`));
        }
        if (req.file)
            data.image = `/uploads/images/projects/${req.file.filename}`;
...
router.post('/:id/request/:userId/approve', authMiddleware, async (req, res) => {
    try {
        const postId = parseInt(req.params.id);
...
router.post('/:id/request/:userId/reject', authMiddleware, async (req, res) => {
    try {
        const postId = parseInt(req.params.id);
        const userId = parseInt(req.params.userId);

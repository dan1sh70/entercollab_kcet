...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({ papers, total, page: parseInt(page), lastPage: Math.ceil(total / take) });
    }
    catch (e) {
...
router.post('/', authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        if (doi)
            data.doi = doi;
        if (publication_date)
            data.publicationDate = new Date(publication_date);
        if (req.file)
            data.filePath = `/uploads/files/research/${req.file.filename}`;
...
router.patch('/:id', authMiddleware, upload.single('file'), async (req, res) => {
    try {
        const paper = await prisma.researchPaper.findUnique({ where: { id: parseInt(req.params.id) } });
        if (!paper || paper.userId !== req.user.id) {

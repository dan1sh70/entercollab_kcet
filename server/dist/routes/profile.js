...
router.patch('/', authMiddleware, upload.single('profile_photo'), async (req, res) => {
    try {
        ...
        if (github_handle !== undefined)
            data.githubHandle = github_handle;
        if (linkedin_handle !== undefined)
            data.linkedinHandle = linkedin_handle;
...
router.get('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const [projects, followersCount, followingCount, projectsCount, isFollowing, researchPapers, collabRequests, researchTotalCount, collabsTotalCount,] = await Promise.all([

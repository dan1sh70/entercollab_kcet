...
router.post('/send-code', async (req, res) => {
    try {
        ...
        if (existing) {
            res.status(422).json({ error: 'This email is already registered. Try signing in.' });
            return;
        }
...
router.patch('/complete-profile', authMiddleware, upload.single('profile_photo'), async (req, res) => {
    try {
        ...
        if (skills !== undefined)
            data.skills = skills;
        if (institution_kind)
            data.institutionKind = institution_kind;
...
router.post('/register-institution', async (req, res) => {
    try {
        ...
        if (!name || !email || !password || !institution_kind) {
        ...
        let college = await prisma.college.findFirst({ where: { name } });
        if (!college) {
            college = await prisma.college.create({ data: { name, slug: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), isVerified: 1 } });
        }
        const hashed = await bcrypt.hash(password, 12);
